...
# iterates over the neighbouring nodes of the current board
def nodes(current):
        nodes = []
        for row in range(len(current)):
            for column in range(len(current)):
                if column != current[row]:
                    node = list(current)
                    node[row] = column
                    nodes.append(tuple(node))
        return nodes

# steepest ascent version of the hill climbing algorithm
def hill_climbing(board):
    
    i = 0
    solution = (len(board) - 1) * len(board) / 2

    # evaluate the initial state, if it is the solution then return it
    if evaluate_state(board) == solution:
        print('Solved puzzle!')
        print('Final state is:')
        print_board(board)

    # else set the current state to the initial state
    current = board

    # loop until solution state is found
    while current != solution:
        ...
        # iterates through the neighbours of the current board
        neighbours = nodes(current)
        # find the neighbour with the highest value
        neighbour =  max(neighbours,
                key=lambda state: (evaluate_state(current), random.random()))

        # if the neighbour has higher heuristic value than the current state
        if(evaluate_state(neighbour) >= evaluate_state(current)):
            # then set the current state as the neighbour state
             current = neighbour
    
        # If the current state is the solution then exit
        if evaluate_state(current) == solution:
            print('Solved puzzle!')
            break
...
# schedule function that maps time to temperature 
def schedule(k=0.1, lam=0.001, limit=20000):
    return lambda t: (k * math.exp(-lam * t) if t < limit else 0)

# simulated annealing is a version of stochastic hill climbing
def simulated_annealing(board):
    ...
    # initialise neighbour as a copy of the current baord
    neighbour = list(current)

    # current value is the heuristic value of the current board
    currentValue = evaluate_state(current)
    ...
    while current != solution:
        i += 1
        print('iteration ' + str(i) + ': evaluation = ' + str(evaluate_state(board)))
        if i == 10000:  # Give up after 10000 tries.
            break
        
        # iteratively increase time
        t+=1

        # value of temperature as a function of time determined with the schedule function
        T = schedule(t)

        # if T = 0 or solution is found exit
        if T == 0 or evaluate_state(current) == solution:
            print('Solved puzzle!')
            break
        
        # neighbour is a randomly selected successor of current 
        for column, row in enumerate(current):  # For each column, place the queen in a random row
            neighbour[column] = random.randint(0, len(board)-1)
    
        if not neighbour:
            print('Solved puzzle!')
            break
        
        # the value of next is the heuristic value of the neighbour states
        nextValue = evaluate_state(neighbour)
